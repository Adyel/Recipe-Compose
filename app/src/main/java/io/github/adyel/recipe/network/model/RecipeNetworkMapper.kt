...
import io.github.adyel.recipe.domain.util.EntityMapper
...
class RecipeNetworkMapper : EntityMapper<RecipeNetworkEntity, Recipe> {
    ...
    fun fromEntityList(recipeNetworkEntityList: List<RecipeNetworkEntity>): List<Recipe>{
        return recipeNetworkEntityList.map { mapFromEntity(it) }
    }

    fun toEntityList(recipeList: List<Recipe>): List<RecipeNetworkEntity>{
